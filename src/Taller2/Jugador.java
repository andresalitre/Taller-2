...
public class Jugador {
	...
	public void team() { // Printea el equipo del jugador, con lo que se pide, stats, nombre tipo,
							// tambien agregue estado
		if (equipo.size() == 0) {
		...
		for (int i = 0; i < equipo.size(); i++) {
			if (j < 6) {
				...
				System.out.println("-" + p.getNombre() + "|" + p.getTipo() + "|Stats: " + p.sumaStats() + "|"
						+ equipo.get(i).getEstado());
				j++;
			}
		}
		System.out.println();
	...
	public void curarEquipo(List<PokemonJugador> equipo) {

		if (equipo.size() == 0) {
			System.out.println("no tienes tonto");
			return;
		} else {
			for (int i = 0; i < equipo.size(); i++) {
				equipo.get(i).curar();
	...
	} // Se crea un auxiliar para guardar el que sale, se cambian las posiciones y
		// luego se agrega el auxiliar
